        ...
        if x == 0:
            ...
            while countProbK <= 0.999:
                ...
                for i in range(start, len(probK[x - 1])):
                    ...
                    pj = (
                        pj
                        + (numComb * (ps ** j) * (pf ** (i * parameter.NODES - j)))
                        * probK[x - 1][i]
                    )
                if j <= parameter.NODES:
                    ...
                    pj = (
                        pj
                        + (numComb * (ps ** j) * (pf ** (parameter.NODES - j)))
                        * probK[x - 1][0]
                    )
                    """z = 1
                    pz = 0
    ...
    if r >= 1:
        ...
        while k <= len(pk[r - 1]) - 1:
            ...
            for i in range(0, k):
                ...
                numComb = Combinations(k, i)
                numRound_i = numRound_i + numComb * ((psn) ** (k - i)) * (
                    (1 - psn) ** i
                )

            pf_i = pk[r - 1][k] * pf_i
            pf_r = pf_r + pf_i
            NumRound = NumRound + pk[r - 1][k] * int(
                math.ceil(float(1) / float(numRound_i))
            )
            k = k + 1
...
def main():
    ...
    pnf = (pf ** parameter.NODES) + (
        parameter.NODES * ps * (pf ** (parameter.NODES - 1))
    )
    print(pnf)
