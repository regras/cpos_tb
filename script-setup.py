...
os.system("rm script-output.out")
os.system("docker service rm cpos_cpos > script-output.out")
check = os.path.getsize('script-output.out')
if check > 0:
    time.sleep(20)

#criacao de novos containers
os.system("docker network rm netppos")
os.system("docker image rm cpos")
os.system("docker network create --driver overlay --subnet 10.2.0.0/22 --gateway 10.2.0.1 netppos")
