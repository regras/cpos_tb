...
class Node(object):
    ...
    def neighbors(self, ipaddr=None, stakeList=None, firstC=True, pPeers=0):
        ...
        while peer < limitP:
            ...
            currentRound = int(
                math.floor(
                    (float(nowTime) - float(parameter.GEN_ARRIVE_TIME))
                    / parameter.timeout
                )
            )
            ...
            for i in peers:
                if i != ipaddr and i not in self.connectPeers:
                    ...
                    distance = int(hashlib.sha256(str(ipaddr)).hexdigest(), 16) ^ int(
                        hashlib.sha256(peerSalt).hexdigest(), 16
                    )
            ...
            print(
                "dif time: ",
                parameter.timeout
                - (time.mktime(datetime.datetime.now().timetuple()) - nowTime),
            )
            if (
                time.mktime(datetime.datetime.now().timetuple()) - nowTime
                < parameter.timeout
    ...
    def startThreads(self):
        ...
        listenInsert_thread = threading.Thread(
            name="listenInsert", target=self.listenInsert
        )
        ...
        currentRound = int(
            math.floor(
                (float(nowTime) - float(parameter.GEN_ARRIVE_TIME)) / parameter.timeout
            )
        )
        ...
        print(
            "Sleeping time: ",
            parameter.timeout - ((nowTime - parameter.GEN_ARRIVE_TIME) - currentRound),
        )
        time.sleep(
            parameter.timeout - ((nowTime - parameter.GEN_ARRIVE_TIME) - currentRound)
        )
    ...
    def stableBlock(self):
        ...
        if not self.threadSync.is_set():
            ...
            self.lround, sync, self.commit = sqldb.reversionBlock(
                currentRound, self.lround, self.commit
            )
            if not sync:
                self.lastRound = currentRound
                self.resync = 0
    ...
    def commitBlock(self, message=None, t=0):
        ...
        if t == 0:
            ...
            if message:
                ...
                if not status or round <= block.round:
                    ...
                    )  # we need create same block payload in the future
                    userHash, blockHash = self.cons.POS(
                        lastBlock_hash=block.hash, round=round, node=self.node, tx=tx
                    )
                    subUser = validations.sortition(userHash, self.stake, self.cons)
                    if subUser > 0:
                        proofHash, prioritySubUser = self.cons.calcProofHash(
                            userHash, blockHash, subUser
                        )
                    else:
        ...
        if t == 6:
            if message:
                ...
                lastForks = message[2]
                numBlocks = message[3]
                lastForks, newBlocks = calculation.calcParameters(
                    self, start, end, lastForks, numBlocks
                )
        ...
        if t == 16:
            ...
            if blocks:
                for item in blocks:
                    checkProof, subUser = validations.validateProofHash(
                        item, stake[1][item.node][0], self.cons
                    )
                    if checkProof:
    ...
    def sendControl(self, message):
        ...
        if self.bestblock:
            if (b.round < self.bestblock[0].round) or (
                (b.round <= self.bestblock[0].round)
                and (b.proof_hash < self.bestblock[0].proof_hash)
            ):
            ...
            currentRound = int(
                math.floor(
                    (
                        ...
                        - float(parameter.GEN_ARRIVE_TIME)
                    )
                    / parameter.timeout
                )
            )
        ...
        while True and not self.k.is_set():
            ...
            if not self.threadSync.is_set():
                ...
                for i, msg in sorted(self.msg_arrivals.items(), key=itemgetter(1)):
                    for j, item in sorted(
                        self.msg_arrivals[i].items(), key=itemgetter(1)
                    ):
                        b = item[0]
                        node = item[1]
                        user_stake = int(item[2])
        ...
        while True and not self.k.is_set():
            ...
            if self.startMine:
                ...
                if nowTime - prevTime < parameter.timeout:
                    print(
                        "ROUND SLEEP TIME: ", parameter.timeout - (nowTime - prevTime)
                    )
    ...
    def syncRequestAllBlocks(self, b, peers_vote, peerS):
        ...
        while index < len(peers_vote):
            ...
            message = self.reqBlock(
                [pickle.dumps(b, 2), "allblocks"], peers_vote[index]
            )
            if message:
    ...
    def syncRequestMajor(self, round, orderpeer, peerS):
        ...
        while not status and index < len(self.peers):
            message = self.reqBlock(
                [pickle.dumps(round, 2), "majorblock"], orderpeer[index][0]
            )
            b = None
            # logblock = None
            if message:
            ...
            try:
                if self.threadSync.is_set():
                    ...
                    while index < parameter.k:
                        ...
                        b = None
                        try:
                            b = self.syncRequestMajorPerPeer(
                                round - parameter.roundTolerancy - 1,
                                orderpeer[index][0],
                            )
                        except Exception as e:
                            print(str(e))
                        if b:
                            print(
                                "proof_hash first block sync function: ", b.proof_hash
                            )
                            ...
                            try:
                                blocks = self.syncRequestAllBlocksPerPeer(
                                    b, orderpeer[index][0]
                                )
                            except Exception as e:
                                print(str(e))
                            ...
                            while status and not sqldb.dbKnowBlock(prev_hash):
                                ...
                                if b:
                                    ...
                                    try:
                                        b = self.syncRequestBlockPerPeer(
                                            prev_hash, orderpeer[index][0]
                                        )
                                    except Exception as e:
                                        print(str(e))
                                    if b:
                            ...
                            if chain:
                                ...
                                if index == 0:
                                    try:
                                        idreversion = sqldb.insertReversion(
                                            round, lastround
                                        )
                                    except Exception as e:
                                        print(str(e))
                                if not know and checkProof:
                                    ...
                                    bestchain = True
                                    if self.firstsync == 0:
                                        currentsuc = sqldb.getCurrentSuc(
                                            lastround,
                                            (round - parameter.roundTolerancy - 1),
                                        )
                                    ...
                                    if bestchain:
                                        ...
                                        try:
                                            if idreversion:
                                                sqldb.addBlocksReversion(
                                                    fblock, lblock, idreversion
                                                )
                                        except Exception as e:
                                            print(str(e))
                                        t = max(chain)
                                        ...
                                        self.semaphore.release()
                                        print(
                                            "PEERS HAVE A WORST CHAIN...WE WILL NEED CONNECT WITH MORE PEERS AND TRY AGAIN"
                                        )
                                        # self.resync = self.resync + 1
                                else:
                                    # self.semaphore.release()
                                    print(
                                        "PEERS BELIEVE ON THE SAME CHAIN OR CHAIN IS NOT CORRECT."
                                    )
                                    # self.resync = self.resync + 1
                            else:
                                # self.semaphore.release()
        ...
        while index and trials:
            ...
            if new and validations.validateBlockHeader(new):
                ...
                if validations.validateBlock(
                    new, pblock
                ) and validations.validateChallenge(new, self.stake):
    ...
    def messageHandler(self):
        ...
        while True and not self.k.is_set():
            ...
            if (
                messages[0].lower() == consensus.MSG_REQBLOCK
                and self.threadSync.is_set()
            ):
                reply = None
    ...
    def rpcServer(self, ip="127.0.0.1", port=9999):
        ...
        while True:
            ...
            if cmd == rpc.MSG_LASTBLOCK:
                ...
                msg_start_peers = threading.Thread(
                    name="startnode",
                    target=self.startnode,
                    kwargs={"ipaddr": self.ipaddr, "startTime": startTime},
                )
                ...
                print(
                    "Sleeping time: ",
                    parameter.timeout
                    - ((nowTime - parameter.GEN_ARRIVE_TIME) - currentRound),
                )
                time.sleep(
                    parameter.timeout
                    - ((nowTime - parameter.GEN_ARRIVE_TIME) - currentRound)
                ...
                last_hash = sqldb.dbtoBlock(
                    sqldb.blockQuery(["", str(int(l[0]) - 1)])
                ).hash
                ...
                b = block.Block(
                    int(l[0]), last_hash, int(l[1]), hash_node, time_create, hash
                )
                sqldb.writeBlock(b)
                # sqldb.writeChain(b)
                ...
                self.psocket.send_multipart(
                    [consensus.MSG_BLOCK, self.ipaddr, pickle.dumps(b, 2)]
                )
            else:
    ...
    def reqLastBlock(self):
        ...
        self.reqsocket.send_multipart(
            [consensus.MSG_LASTBLOCK,]
        )
        ...
        if address:
            ...
            try:
                self.router.send_multipart(
                    [consensus.MSG_REQPEER, str(salt), self.ipaddr]
                )
                m = self._poll(self.router)[0]
            except Exception as e:
                print(str(e))
    ...
    def reqBlock(self, message, address=None):
        ...
        if address:
            ...
            try:
                self.router.send_multipart(
                    [consensus.MSG_REQBLOCK, message[0], str(message[1])]
                )
                m = self._poll(self.router)[0]
            except Exception as e:
                print(str(e))
    ...
    def reqBlocksChain(
        self, localHash, remoteTargetHash, address=None,
    ):
        if address:
            self.router.connect("tcp://%s:%s" % (address, self.port + 1))
            print("IP")
            ...
            time.sleep(1)
            self.router.send_multipart(
                [consensus.MSG_BLOCKCHAIN, str(localHash), str(remoteTargetHash)]
            )
            m = self._poll(self.router)[0]
            self.router.disconnect("tcp://%s:%s" % (address, self.port + 1))
        else:
    ...
    def reqSendHeadsChains(self, forkHash, headChains, blockHash, address=None):
        if address:
            ...
            self.router.send_multipart(
                [
                    consensus.MSG_REQBLOCKS,
                    str(forkHash),
                    pickle.dumps(headChains),
                    str(blockHash),
                ]
            )
            m = self._poll(self.router)[0]
...
def main():
    ...
    parser.add_argument(
        ...
        metavar="fmine",
        dest="fmine",
        help="Specify a mine node",
        default="True",
    )
    parser.add_argument(
        "--stake", dest="stake", nargs="*", help="Specify stake user", default=[]
    )
    parser.add_argument(
        "--peers",
        dest="peers",
    ...
    msg_start_peers = threading.Thread(
        name="startnode",
        target=n.startnode,
        kwargs={"ipaddr": args.ipaddr, "startTime": startTime},
    )
    msg_start_peers.start()
